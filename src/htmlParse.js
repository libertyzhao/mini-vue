...
function parseDom(html, Vnode) {
  var result = html.match(startTagOpen),target = null;;
  if (result[0] == html) {//整个html解析结束
    Vnode.parent.children.pop();//删除当前无用节点
    return {html:''};
  } else if (result[1]) {//如果是结束标签</div>，则当前的vnode一定是div标签的子元素
...
function stackAdd(Vnode) {
  var children = Vnode.parent.children,index = children.length,target=null;
  children[index] = createVnode();
  target = children[index];
